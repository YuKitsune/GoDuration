...
/// <summary>
/// How the microsecond unit is rendered. <see cref="Mu"/> is the Go default.
/// </summary>
...
/// <summary>
/// Formatting knobs for <see cref="Duration.Format(TimeSpan, DurationFormatOptions)"/>.
...
public readonly record struct DurationFormatOptions
{
    ...
    /// <summary>When true, positive non-zero values are prefixed with <c>+</c>.</summary>
    public bool IncludePositiveSign { get; init; }

    /// <summary>Which glyph to use for the microsecond unit.</summary>
    public MicrosecondSymbol MicrosecondSymbol { get; init; }

    /// <summary>
    /// When true, zero-valued unit segments in the seconds-and-above rendering are dropped.
    /// A total-zero value still emits <c>"0s"</c>. Sub-second output is single-segment and
    /// unaffected.
    /// </summary>
